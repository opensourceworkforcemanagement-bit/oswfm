...
CREATE TABLE resources (
    ...
    resource_type VARCHAR(100) NOT NULL,  -- e.g., 'document', 'database', 'api'
...
CREATE TABLE attribute_definitions (
    ...
    attribute_category VARCHAR(100) NOT NULL,  -- 'subject', 'resource', 'environment', 'action'
    data_type VARCHAR(50) NOT NULL,  -- 'string', 'number', 'boolean', 'datetime', 'list'
    description TEXT,
...
-- Actions table - defines possible operations
CREATE TABLE actions (
    action_id SERIAL PRIMARY KEY,
    action_name VARCHAR(100) UNIQUE NOT NULL,  -- e.g., 'read', 'write', 'delete', 'execute'
    description TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- Action attributes
CREATE TABLE action_attributes (
    action_attr_id SERIAL PRIMARY KEY,
    action_id INTEGER NOT NULL REFERENCES actions(action_id) ON DELETE CASCADE,
    attribute_id INTEGER NOT NULL REFERENCES attribute_definitions(attribute_id) ON DELETE CASCADE,
    attribute_value TEXT NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(action_id, attribute_id)
);
...
CREATE TABLE policies (
    ...
    description TEXT,
    policy_type VARCHAR(50) NOT NULL,  -- 'permit', 'deny'
...
CREATE TABLE policy_resource_targets (
    ...
    resource_type VARCHAR(100),  -- Can specify type instead of specific resource
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    CHECK ((resource_id IS NOT NULL) OR (resource_type IS NOT NULL))
);

-- Policy target actions - which actions does this policy apply to
CREATE TABLE policy_action_targets (
    target_id SERIAL PRIMARY KEY,
    policy_id INTEGER NOT NULL REFERENCES policies(policy_id) ON DELETE CASCADE,
    action_id INTEGER NOT NULL REFERENCES actions(action_id) ON DELETE CASCADE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE policy_obligations (
    ...
    policy_id INTEGER NOT NULL REFERENCES policies(policy_id) ON DELETE CASCADE,
    obligation_type VARCHAR(100) NOT NULL,  -- 'log_access', 'notify_admin', 'encrypt_data'
...
-- Attributes
CREATE INDEX idx_attribute_definitions_category ON attribute_definitions(attribute_category);
...
CREATE INDEX idx_policy_action_targets_policy ON policy_action_targets(policy_id);
CREATE INDEX idx_policy_action_targets_action ON policy_action_targets(action_id);
...
CREATE INDEX idx_access_requests_action ON access_requests(action_id);
